...
func NewTxBuilder(provider string, privateKey *ecdsa.PrivateKey, chainID *big.Int) (TxBuilder, error) {
	...
	tokenAddr, tx, token, err := contract.DeployERC20BurnableMock(auth, client, "USDC coin", "USDC", auth.From, big.NewInt(1e18))
	if err != nil {
		return nil, err
	}
...
func (b *TxBuild) TransferERC20Token(ctx context.Context, to string, value *big.Int) (common.Hash, error) {
	tx, err := b.token.Transfer(common.HexToAddress(to), value)
	if err != nil {
		return common.Hash{}, err
	}
	return tx.Hash(), nil
